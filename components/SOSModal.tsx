...
export const SOSModal: React.FC<SOSModalProps> = ({ isOpen, onClose, language }) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-6 animate-in fade-in zoom-in duration-300 ${fontClass}`} style={{ backgroundColor: THEME.danger }}>
      <button 
        onClick={onClose}
        className="absolute top-6 right-6 text-white bg-white/20 p-2 rounded-full hover:bg-white/30 transition z-50"
      >
        <X size={32} />
      </button>

      {/* Main Content Scrollable Container */}
      <div className="flex flex-col items-center w-full max-h-screen overflow-y-auto py-10">
        <div className="text-center text-white mb-10 shrink-0">
        ...
        <div className="w-full max-w-md bg-white rounded-2xl p-6 shadow-2xl shrink-0">
